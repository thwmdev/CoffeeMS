...
# =========================
# LỊCH SỬ CHỈNH SỬA & ĐƠN BẤT THƯỜNG (UC06 – Bước 4)
# =========================
...
def get_edit_history():
    ...
    try:
        ...
        # Đơn bất thường – chỉnh sửa > 5 lần
        cursor.execute(
            f"""
            SELECT
                LS.MaDon,
...
def export_pdf():
    ...
    try:
        ...
        if not rows:
            return "Không có dữ liệu để xuất báo cáo.", 404
        ...
        format_tu  = datetime.datetime.strptime(tu_ngay, "%Y-%m-%d").strftime("%d/%m/%Y")
        format_den = datetime.datetime.strptime(den_ngay, "%Y-%m-%d").strftime("%d/%m/%Y")
        ngay_lap   = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
...
<style>
  body{{font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#333;margin:0;padding:20mm 15mm;}}
  .header-table{{width:100%;border:none;margin-bottom:20px;}}
  .company-title{{font-size:14px;text-transform:uppercase;font-weight:bold;}}
  .report-title{{text-align:center;color:#2c3e50;margin:25px 0 10px;font-size:22px;font-weight:bold;text-transform:uppercase;}}
  ...
  .signature-section{{width:100%;margin-top:50px;border:none;}}
...
<table class="header-table">
  <tr>
    <td class="company-title">☕ COFFEE MANAGEMENT SYSTEM<br><span style="font-size:11px;font-weight:normal;color:#666;">Hệ thống quản lý quán Cafe</span></td>
    <td style="text-align:right;font-style:italic;color:#718096;">Ngày lập: {ngay_lap}</td>
  </tr>
</table>

<div class="report-title">Báo cáo Doanh thu Bán hàng</div>
<div class="sub-title">Từ ngày {format_tu} đến ngày {format_den}</div>
